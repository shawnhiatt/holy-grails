...
- **Play history date + time picker** — date picker now includes a time input alongside the date field; both pre-fill to current date and time on open. Log button is the sole trigger for writing a play record — selecting a date or time only updates local state. Reset button restores both fields to current date and time.
- **Delete play history entries** — individual play records can be deleted via swipe-to-delete, consistent with the Sessions screen pattern. `deletePlay` Convex mutation added with auth gating and ownership check. Play count pill updates reactively via existing `getByUsername` subscription invalidation.
