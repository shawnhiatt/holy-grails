...
src/
  app/
    ...
    components/
      ...
      stack-builder.tsx  # Session Builder sheet — creates a session that fills itself. Presets only in v1 (generated from the real collection via utils/stack-presets.ts, so nothing on offer comes back empty). Names a tool, not a thing: all copy says "Session", the mechanism is a verb ("this session fills itself"), never "smart"/"folder"/"playlist".
