...
- **The "N for sale" text is the one sanctioned outbound Discogs link** (`/sell/release/{id}`). It must go through the `/api/go` redirector (see below) — never a raw `discogs.com` href. Reach to `AlbumDetailPanel`/`WantItemDetailPanel` is a deliberate future decision, not shipped.

**Outbound Discogs links & the `/api/go` redirector.** Tapping a raw `discogs.com` link inside the installed PWA triggers the Discogs app's iOS Universal Link and strands the user on its home screen. A client-side redirector page (the former `public/go.html`) did NOT fix this — the in-app browser treats a JS `location.replace()` as a fresh navigation, app-switches anyway, and strands the overlay on the blank intermediate page (requiring a force-close). The redirect must be SERVER-SIDE: `api/go.js` is a Vercel function — links point at `/api/go?u={encoded url}` (same-origin tap, no Universal Link), and the function validates `u` against a `https://www.discogs.com/` allowlist and issues an HTTP 302. iOS evaluates Universal Links only against the URL the user tapped; server redirects don't re-trigger the check. The route is already excluded from the SW navigation fallback via the `/api/` denylist entry in `vite.config.ts`. Note: `/api/go` only works on Vercel deployments — it 404s under `npm run dev` (Vite doesn't run Vercel functions). This is the required pattern for any outbound Discogs link. The blanket "View on Discogs" link removal otherwise stands — the Value section listings link is the single exception.
...
- **Record-store price lookup** — Value section in `ReleaseDetailPanel` (lowest ask + N-for-sale + VG/VG+/NM suggestions), with an outbound listings link via the `/api/go` server-side 302 redirector (avoids the iOS PWA→Discogs-app Universal Link trap; a client-side redirect page was tried first and failed).
