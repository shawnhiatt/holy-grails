...
- User preferences (theme, hide purge indicators, shake to random, view mode, want view mode, default screen, default collection sort, recent Look It Up searches, format scope), keyed by `discogs_username`. The `hide_gallery_meta` field still exists in the schema but is legacy — its swiper view was removed and the Settings toggle deleted with it; do not resurface it.
...
### Formats (all-formats)
Holy Grails syncs **every media format Discogs supports** (vinyl, CD, cassette, shellac, box sets, files, …). Scope is a **display-only** concern, never a data-layer filter (see `docs/all-formats-plan.md`). The data layer stores everything; `syncSelf`/`syncFollowedUser` do not filter by format, and the caches are faithful mirrors of Discogs.
...
src/
  app/
    ...
    utils/
      ...
      insights.ts        # Pure derivations for the Insights screen: parseAddedYear, bucketAddsByYear (Collection Growth). No React/recharts — testable in node env. (Spending's parsePricePaid/deriveSpending were removed — pricePaid is never populated by sync.)
...
- **Panel section order**: Hero → Thumbnail carousel → Purge tag → Your Copy (with section header, Format, Label, Catalog #, Year, Country, Folder, Media, Sleeve, Paid, custom fields) → User Notes → Mark as Played → Community (compact row) → Value → Enriched Tabs (mobile) or accordion sections (desktop) → Sessions → Rate for Purge. `AlbumDetailPanel` renders the shared **Value section** (below) right after Community, same as `ReleaseDetailPanel` — it reuses the live `lowestPrice`/`numForSale` the panel already fetches via `proxyFetchRelease`, so there's no extra request. (This live ask can differ from the ~monthly drip value shown in Insights Top Shelf; the album-detail figure is the current one.)
...
*(A **Spending** section — total/avg/most-expensive from `pricePaid` — was removed: `pricePaid` is hardcoded `""` at sync (`convex/discogs.ts`) and never populated, so it was permanently inert. Do not rebuild it or any feature on `pricePaid`; Discogs exposes price paid only as a per-user custom field, not universal data. `parsePricePaid`/`deriveSpending` were deleted from `utils/insights.ts` with it.)*
